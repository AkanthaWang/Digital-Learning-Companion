...
class FunASRModule:
    ...
    def transcribe(
        ...
        try:
            ...
            logger.debug("开始识别音频: {}".format(
                audio_input if isinstance(audio_input, str) else "numpy array (shape: {})".format(
                    np.array(audio_input).shape if isinstance(audio_input, (np.ndarray, list)) else "unknown"
                )
            ))
